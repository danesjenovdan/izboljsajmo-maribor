...
def send_email_task(subject, email, template, data, from_email=settings.FROM_EMAIL):
    send_email(subject, email, template, data, from_email)
...
def send_daily_notifications():
    ...
    notifications = Notification.objects.filter(is_sent=False).prefetch_related(
        "initiative", "for_user"
    )

    for_superadmin = notifications.filter(for_user__role=Reviwers.SUPER_ADMIN).distinct(
        "initiative"
    )
    ...
    comments = (
        Comment.objects.filter(created__gte=datetime.now() - timedelta(days=1))
    ...
    if comments:
        ...
        for super_admin in super_admins:
            send_email(
                ...
                {"comments": comments, "base_url": settings.BASE_URL},
            )
    ...
    for notification in for_non_superadmin:
        ...
        if notification.for_user:
            ...
            area_admins = User.objects.filter(area=area, role=Reviwers.AREA_ADMIN)
            print(area_admins)
    ...
    for email, initiatives in email_initiatives.items():
        send_email(
            ...
            {"initiatives": initiatives, "base_url": settings.BASE_URL},
        )
