...
from initiatives.models import (  # users
    AreaAdminUser,
    AreaAppraiserUser,
    BasicUser,
...
from initiatives.utils import id_generator, send_email
...
def check_if_area_is_changed(sender, instance, **kwargs):
    ...
    if old_instance.area != instance.area:
        ...
        Notification(
            for_area=instance.area, initiative=instance, type=NotificationType.UPDATED
        ).save()
        Notification.objects.filter(
            for_area=old_instance.area, initiative=instance
        ).delete()
...
def set_key(sender, instance, created, **kwargs):
    if created:
        ...
        send_email_task.delay(subject, instance.user.email, template, {"url": url})
...
def send_status_initiative_response(sender, instance, created, **kwargs):
    if (
        instance.publication_status == Published.PUBLISHED
        and not instance.is_email_sent
    ):
...
def check_is_blocked(sender, instance, created, **kwargs):
    ...
    if instance.note != instance._old_note:
        ...
        send_email_task.delay(
            ...
            {"username": instance.username, "note": instance.note},
        )
...
post_save.connect(
    notify_superadmin_for_new_status_initiative, sender=StatusInitiativeHear
)
post_save.connect(
    notify_superadmin_for_new_status_initiative, sender=StatusInitiativeEditing
)
post_save.connect(
    notify_superadmin_for_new_status_initiative, sender=StatusInitiativeProgress
)
post_save.connect(
    notify_superadmin_for_new_status_initiative, sender=StatusInitiativeFinished
)
post_save.connect(
    notify_superadmin_for_new_status_initiative, sender=StatusInitiativeDone
)
post_save.connect(
    notify_superadmin_for_new_status_initiative, sender=StatusInitiativeRejected
)
